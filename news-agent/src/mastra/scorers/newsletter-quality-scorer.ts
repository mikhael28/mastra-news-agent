...
export const newsletterQualityScorer = createScorer({
  ...
  .analyze({
    ...
    createPrompt: ({ results }) => `
      ...
      Also provide:
      - overallScore: Your overall quality rating (1-10). A 7 means "acceptable for publication."
      ...
      - improvementSuggestion: If overallScore < 7, suggest how to refine the topic for a better result
      ...
      messages: [
        {
          ...
          content:
            ...
            "improvementSuggestion (string - if overallScore < 7, restate the original topic in a " +
            "more focused way; MUST be 80 characters or fewer, no explanation, just the restated topic).",
        },
        {
          role: "user",
          content: `Topic: ${topic}\n\nNewsletter:\n${markdown}`,
